...
//
// ── 主要ロケーション座標リファレンス（データ追加時に参照） ──
// マーカーは「事案発生海域・空域の中心」に置く。陸名で代替しない。
//
//   尖閣諸島領海:        25.74, 123.47
//   尖閣諸島接続水域:    25.74, 123.5
//   宮古海峡:            25.0, 125.5
//   沖縄本島・宮古島間:  25.5, 127.5
//   与那国島南方:        24.0, 123.0
//   台湾海峡 西側:       23.0, 119.8（福建沖／南端）
//   台湾海峡 中央:       24.5, 119.5
//   台湾海峡 北端西側:   25.0, 119.8
//   台湾ADIZ 西南部:     22.5, 119.5
//   台湾東岸沖（ADIZ東部）: 23.5, 121.0
//   日本海中央(中露合流): 37.5, 134.5
//   オホーツク海:        47.0, 145.0（樺太の東）
//   津軽海峡:            41.5, 140.5
//   宗谷海峡:            45.5, 141.5
//   根室半島沖:          43.2, 146.0
//   知床岬東方:          44.0, 146.5
//   韓国KADIZ南方:       33.0-34.0, 127.0-128.5
//   北朝鮮西岸 発射:     39.0, 125.0〜126.5（陸でOK＝missile）
//   北朝鮮東部 着弾:     39.0-41.0, 130.0-132.0（日本海EEZ外）
//   スカボロー礁:        15.18, 117.78
//   仁愛礁:              9.7, 115.85
//   サビナ礁:            9.85, 115.85
//   西沙諸島周辺:        16.5, 112.0
//   青島港東方 黄海:     36.0, 120.5（中国沿岸都市は港湾沖を使う）
//
// 上記に該当しない新規座標は、地図で実際の位置を確認してから入れる。
// LAND bbox に引っかかった場合、本当に海上なら SEA_OVERRIDES に追加。
const fs = require('fs');
...
const SEA_OVERRIDES = new Set([
  ...
  '41,131.5',   // 能登半島北方
  '40.5,138.5', // 秋田県沖日本海
  '41.5,138',   // 渡島大島西方EEZ（火星-17 落下点）
  // 台湾周辺
  '23,119.8',   // 台湾海峡西側
  '25,122',     // 旧「台湾海峡北端」表記。実際は台湾北東沖太平洋なので新規利用は非推奨
  '25.2,120',   // 台北西方海峡
  '24.5,119.5', // 台湾海峡中央
  '25,119.8',   // 台湾海峡北端（西側・福建沖）
  '26,120.5',   // 馬祖列島南方海上（台湾海峡北端東側）
  // 中国大陸沿岸の港湾（bbox 内だが実際は海上）
  '36,120.5',   // 青島港東方 黄海
  '36.5,121',   // 黄海中央北部
]);
